        ...
        public static bool TryGetMessage(Direction direction, string payloadData, out object message) 
        {
            ...
            try
            {
                if (direction == Direction.Sent)
                    message = JsonSerializer.Deserialize<BaseMessage>(payloadData);
                else
                    message = JsonSerializer.Deserialize<BaseWSResponse>(payloadData);

                //TODO: parse data/result from classes
                return true;
            }
            catch (Exception _)
            {
